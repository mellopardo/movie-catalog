...
const ExploreMore = ({ location }) => {
    ...
    const getData = async () => {
        let url = category ? `https://api.themoviedb.org/3/${type}/${category}/${filter}` : `https://api.themoviedb.org/3/${type}/${filter}`;
        const response = await fetch(`${url}?api_key=cdb0602d337edb45bd801723204229fd&language=en-US`)
    ...
    return (
        <Page>
            ...
            <div className='grid-results'>
                {categoryType && categoryType.results.map(item => (
