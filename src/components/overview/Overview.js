...
const Overview = ({ title }) => {
  ...
  const getData = async () => {
    const castResponce = await fetch(
      `https://api.themoviedb.org/3/${type}/${id}/credits?api_key=cdb0602d337edb45bd801723204229fd&language=en-US`
    );
    const castData = await castResponce.json();

    const recommendationResponse = await fetch(
      `https://api.themoviedb.org/3/${type}/${id}/recommendations?api_key=cdb0602d337edb45bd801723204229fd&language=en-US`
    );
