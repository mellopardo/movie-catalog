...
const Card = ({ history, item, type }) => {
    ...
    return (
        <div className='card' onClick={() =>type && history.push(`/${type}/${item.id}`)}>
         <div className='card-img'>
            ...
            <div>
                <h5 className='card-details-name'>{item.title ? item.title : item.name}</h5>
                <div className='card-details'>
                    ...
                    {item.vote_average && (
                        <div className='card-details-rating'>
                            <span><p>{item.vote_average}</p></span>
